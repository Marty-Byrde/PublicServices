...
interface SearchProps {
  ...
  onSubmit: (filter: FilterOption, input: string) => void
}
...
export default function SearchComponent({ filters, onSubmit }: SearchProps) {
  ...
  const FilterOption = ({ label }) => {
    return (
      <li>
        <button onClick={() => {
          setFilter(filters?.indexOf(filters?.find(o => o.label === label)));
          setIsOptionOpen(false)
        }} type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{label}</button>
      </li>
    )
  }
  ...
  const handleSearch = (e?: SyntheticEvent) => {
    e?.preventDefault()
    setIsOptionOpen(false)
    onSubmit(filters?.at(filter), input);
  }
    ...
    <form onSubmit={handleSearch} className='max-w-7xl mx-auto'>
      <div className="flex relative ">
        <button onClick={(e) => setIsOptionOpen(prev => !prev)}
                ...
                focus:ring-gray-100 dark:bg-stone-600 dark:hover:bg-stone-700 dark:focus:ring-gray-700 dark:text-white dark:border-gray-600" type="button">{filters?.at(filter)?.label}
        ...
        <div className={` z-10 ${isOptionOpen ? "absolute" : "hidden"} top-[42px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-stone-600`}>
          <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
            {filters?.map(option => <FilterOption key={option.label} label={option.label}/>)}
          </ul>
        </div>
